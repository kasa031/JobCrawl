...
export class KarriereScraper extends BaseScraper {
  constructor() {
    super();
    
    this.config = {
      name: 'Karriere',
      baseUrl: 'https://www.karriere.no/jobb',
      selectors: {
        jobList: 'article[class*="job"], div[class*="job-card"], article[class*="listing"], div[class*="listing-item"], article, div[data-testid*="job"]',
        title: 'h2[class*="title"], h3[class*="title"], h2, h3, a[class*="title"], a[href*="jobb"]',
        ...
        link: 'a[href*="jobb"], a[href*="/job"], a[class*="job-link"], a[href]',
  ...
  /**
   * Scrape Karriere.no with filters using JavaScript form submission
   */
  ...
  /**
   * Scrape Karriere.no using JavaScript form submission for search
   */
  private async scrapeWithFormSubmission(keywords?: string, location?: string): Promise<ScrapedJob[]> {
    ...
    try {
      ...
      logInfo('Navigating to Karriere.no', { url: this.config.baseUrl });
        ...
        jobElements.forEach((element: Element) => {
          try {
            ...
            const linkSelectors = selectors.link
              ? [selectors.link, 'a[href*="jobb"]', 'a[href*="job"]', 'a[href]']
              : ['a[href*="jobb"]', 'a[href*="job"]', 'a[href]'];
            ...
            if (title && link) {
              ...
              if (!link.startsWith('http')) {
                const baseUrl = 'https://www.karriere.no';
                try {
                  if (link.startsWith('/')) {
      ...
      logInfo(`Karriere.no scraped ${jobs.length} jobs`, { keywords, location });
      ...
      logWarn('Error in Karriere.no form submission scraping', error as Error);
      await this.scraperService.close();
